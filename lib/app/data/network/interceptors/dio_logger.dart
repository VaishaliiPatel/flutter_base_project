...
class DioLogger extends Interceptor {
  ...
  void _printLine([String pre = '', String suf = '╝']) => printLog('$pre${'═' * maxWidth}$suf');
  ...
  void _printBlock(String msg) {
    ...
    for (var i = 0; i < lines; ++i) {
      printLog((i >= 0 ? '║ ' : '') + msg.substring(i * maxWidth, math.min<int>(i * maxWidth + maxWidth, msg.length)));
    }
  }
  ...
  void _printPrettyMap(
    ...
    if (isRoot || isListItem) printLog('║$initialIndent{');
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          printLog('║${_indent(tabs)} $key: $value${!isLast ? ',' : ''}');
        } else {
          printLog('║${_indent(tabs)} $key: {');
          _printPrettyMap(value, tab: tabs);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          printLog('║${_indent(tabs)} $key: ${value.toString()}');
        } else {
          printLog('║${_indent(tabs)} $key: [');
          _printList(value, tabs: tabs);
          printLog('║${_indent(tabs)} ]${isLast ? '' : ','}');
        }
      } else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          printLog('║${_indent(tabs)} $key: $msg${!isLast ? ',' : ''}');
        }
      }
    });

    printLog('║$initialIndent}${isListItem && !isLast ? ',' : ''}');
  }
    ...
    list.asMap().forEach((i, dynamic e) {
      ...
      if (e is Map) {
        if (compact && _canFlattenMap(e)) {
          printLog('║${_indent(tabs)}  $e${!isLast ? ',' : ''}');
        } else {
          _printPrettyMap(e, tab: tabs + 1, isListItem: true, isLast: isLast);
        }
      } else {
        printLog('║${_indent(tabs + 2)} $e${isLast ? '' : ','}');
      }
    });
  }
